...
const Dropdown2: React.FC<IDropdownProps> = ({
  name,
  label,
  modalTitle,
  selectedValue,
  ...
  const showLabelBySelectedValue = (): string => {
    ...
    if (!Array.isArray(selectedValue)) {
      const itemSelected = data.find((p) => p.value.toString().search(`${selectedValue}`) !== -1 );
      if (itemSelected?.value.toString().search(removeUnicode(`${selectedValue}`)) !== -1){
    ...
    for (let i = 0; i < selectedValue.length; i += 1) {
      const item = data.find((p) => p.value.toString().search(`${selectedValue[i]}`) !== -1);
